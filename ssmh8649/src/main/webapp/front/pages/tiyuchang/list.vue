<template>
	...
	<scroll-view scroll-x="true" class="tabView" :style='{"padding":"8rpx 0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"rgba(0,0,0,1)","backgroundColor":"#e2e2e2","borderRadius":"0","borderWidth":"0","width":"100%","borderStyle":"solid","height":"auto"}'>
        	<view class="tab" v-for="(item,index) in categoryList" :key="index" :class="categoryName===item.zhuangtai?'tabActive':''" @tap="categoryClick(item.zhuangtai)">{{item.zhuangtai}}</view>
        </scroll-view>
		...
		<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"#efefef"}'>
			<view @tap="onDetailTap(product)" class="uni-product" :style='{"borderRadius":"8rpx","backgroundColor":"#fff"}' v-for="(product,index) in list" :key="index">
					...
					<text v-if="isAuth('tiyuchang','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
					<text v-if="isAuth('tiyuchang','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
				</view>
			</view>
		</view>
		
		
		<button v-if="isAuth('tiyuchang','新增')" class="add-btn" @click="onAddTap()">新增</button>
...
<script>
	export default {
		...
		async onShow() {
			let res = await this.$api.list('zhuangtai', {page:1,limit:100});
			res.data.list.splice(0,0,{id:0,zhuangtai:'全部'})
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.categoryName!='全部'){
					params.zhuangtai = '%' + this.categoryName + '%'
				}


				let res = await this.$api.list(`tiyuchang`, params);
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('tiyuchang', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				let searchForm = {
					...
					searchForm['zhuangtai'] = '%' + this.searchForm.zhuangtai + '%'
																																												...
																																												let res = await this.$api.list(`tiyuchang`, searchForm);
				// 如果是第一页数据置空
