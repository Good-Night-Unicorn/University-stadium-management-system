<template>
	<view class="content">
		<form class="app-update-pv">
				...
				<view :style='{"width":"180rpx","fontSize":"28rpx","color":"rgba(0, 204, 0, 1)","textAlign":"left"}' class="title">状态</view>
				<input :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0, 204, 0, 1)","borderRadius":"16rpx","color":"rgba(247, 6, 6, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' :disabled="ro.zhuangtai" v-model="ruleForm.zhuangtai" placeholder="状态"></input>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`zhuangtai`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
					...
					return
																																				...
																																				if(this.ruleForm.id){
					await this.$api.update(`zhuangtai`, this.ruleForm);
				}else{
					await this.$api.add(`zhuangtai`, this.ruleForm);
				}
