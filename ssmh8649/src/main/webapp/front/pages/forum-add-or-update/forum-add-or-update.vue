	...
	<view class="container my-publish-pv">
		<form>
			...
			<view class="cu-form-group margin-top" style="text-align:center">
				<button :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,0) inset","backgroundColor":"rgba(0, 204, 0, 1)","borderColor":"rgba(0, 204, 0, 1)","borderRadius":"8rpx","color":"#fff","borderWidth":"1","width":"100%","fontSize":"28rpx","borderStyle":"solid","height":"80rpx"}' @tap="onSubmitTap" class="bg-red margin-tb-sm">确认提交</button>
			</view>
		</form>
	</view>
</template>
...
<script>
	export default {
		...
		async onLoad(options) {
			...
			this.user = res.data;
						            						                                    if(table == `xuesheng`){
                this.username = this.user.xuejihao
            }
                                                                                                                                                                        						            						                                                                                    if(table == `yonghu`){
